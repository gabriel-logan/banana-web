...
export function validateReservation(data: {
  ...
  if (validator.isEmpty(data.startTime)) errors.startTime = "Start time is required"
  if (validator.isEmpty(data.endTime)) errors.endTime = "End time is required"
  if (validator.isEmpty(data.responsible)) errors.responsible = "Responsible is required"

  if (data.coffee) {
    if (data.peopleQuantity === null || validator.isEmpty(String(data.peopleQuantity))) {
      errors.peopleQuantity = "People quantity is required when coffee is selected"
    }
  }
